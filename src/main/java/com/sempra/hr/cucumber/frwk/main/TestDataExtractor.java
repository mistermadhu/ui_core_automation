...
public class TestDataExtractor {
			...
			for (List<String> row : xlRecords) {
				...
				if (!isHeader) {
					...
					if (!testDataMap.containsKey(key))
						...
						logger.debug("Duplicate key = [" + key + "] in Sheet [" + sheetName + "] inside Workbook ["
								+ this.xlsFile + "]");
				}
				isHeader = false;
				logger.debug(
	...
	public void populateCommonData() {
		...
		try {
			...
			List<String> valueList;
			boolean isHeader = true;
			logger.debug("\nvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv[" + FrameworkConstants.TEST_DATA_SHEET_COMMON
					+ "]vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv");
			for (List<String> row : xlRecords) {
				if (row == null || row.size() == 0)
					continue;
				...
				if (!isHeader) {
					...
					if (!commonDataMap.containsKey(key))
						...
						logger.debug(
								"Duplicate key = [" + key + "] in Sheet [" + FrameworkConstants.TEST_DATA_SHEET_COMMON
										+ "] inside Workbook [" + this.xlsFile + "]");
				}
				isHeader = false;
				logger.debug(
						"																																			");

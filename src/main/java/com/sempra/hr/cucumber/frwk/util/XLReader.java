...
public class XLReader {
	...
	public List<List<String>> readExcel(String sheetName, boolean isTestScriptDataSheet) throws IOException {  //isTestScriptDataSheet- Sheets with Test Script IDs, Scenario IDs, Iteration No and SkipTest column
			  ...
			  // Read sheet inside the workbook by its name
				...
				for (int i = 0; i < rowCount + 1; i++) {
					...
					for (int j = 0; j < row.getLastCellNum(); j++) {
		               ...
		               {
		                if(j==0&&celVal.length()!=0&&celVal.equalsIgnoreCase("TRUE"))  //SkipType=TRUE will skip, rest will get executed.
		                {
		                	row.getCell(1).setCellType(CellType.STRING);
		                	Cell cell2=row.getCell(2);
		                	if(cell2!=null)cell2.setCellType(CellType.STRING);
		                	Cell cell4=row.getCell(4);
		                	if(cell4!=null)cell4.setCellType(CellType.STRING);
		                	logger.debug("[[ SKIPPED==========>>["+row.getCell(1).getStringCellValue()+"-"+((cell2!=null)?cell2.getStringCellValue():FrameworkConstants.EMTPY_LABEL)+"-"+((cell4!=null)?cell4.getStringCellValue():FrameworkConstants.EMTPY_LABEL)+"]");
		                	break;
		                }
						if(j==2&&celVal.length()==0) // If the iterationNo column is empty, then assign it to 1
						{
							recordList.add("1");
						}
		                }
					   ...
					   else
					   {
						   if(isTestScriptDataSheet)
			               {  
						   if(j==2) // If the iterationNo column is empty, then assign it to 1
							{
								recordList.add("1");
							}
			               }
 ...
 public void closeWorkbook()
 {
	...
	public static void main(String... strings) throws IOException {
		...
		//XLReader objExcelFile = new XLReader("src/test/resources/testdata/", "old_Eligibility_TestData.xls");
		...
		//objExcelFile.readExcel(FrameworkConstants.TEST_DATA_SHEET_COMMON, true);

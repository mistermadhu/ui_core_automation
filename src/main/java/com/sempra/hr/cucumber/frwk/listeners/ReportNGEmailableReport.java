...
/**
 * This class represents a custom emailable report with summary and other
 * information.
 * 
 * @author maritakula@prokarma.com
 * @version 1.0
 */
public class ReportNGEmailableReport implements IReporter {
	private static final Logger LOG = Logger.getLogger(ReportNGEmailableReport.class);
	...
	private static String outFilename = "extended-emailable-report.html";
		...
		if (!classResults.isEmpty()) {
			...
			for (ClassResult classResult : classResults) {
				...
				for (MethodResult methodResult : classResult.getMethodResults()) {
					...
					String methodNameStr = extractMethodNameFromResult(aResult);
					String methodName = Utils.escapeHtml(methodNameStr);
	...
	private int writeScenarioDetails(List<ClassResult> classResults, int startingScenarioIndex) {
		...
		for (ClassResult classResult : classResults) {
			...
			for (MethodResult methodResult : classResult.getMethodResults()) {
				...
				String methodName = extractMethodNameFromResult(mResult);
				String label = Utils.escapeHtml(className + "#" + methodName + " ( " + mResult.getName() + " )");
	...
	protected static class TestResult {
		...
		protected List<ClassResult> groupResults(Set<ITestResult> results) {
			...
			if (!results.isEmpty()) {
				...
				String scenarioName = (String) result.getAttribute("cukes.scenarioName");
				if (!org.apache.commons.lang3.StringUtils.isEmpty(scenarioName)) {
					previousMethodName = scenarioName;
				}
				...
				while (resultsIterator.hasNext()) {
					...
					if (!previousClassName.equals(className)) {
						...
						scenarioName = result.getAttribute("cukes.scenarioName").toString();
						if (!org.apache.commons.lang3.StringUtils.isEmpty(scenarioName)) {

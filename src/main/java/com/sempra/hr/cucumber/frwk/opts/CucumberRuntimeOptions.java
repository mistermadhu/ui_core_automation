...
public class CucumberRuntimeOptions {
	...
	public RuntimeOptions getCucumberRuntimeOptions() throws Throwable {
	   ...
	   /* ClassFinder classFinder = new ResourceLoaderClassFinder(resourceLoader, classLoader);
	    this.runtime = new cucumber.runtime.Runtime(resourceLoader, classFinder, classLoader, ro);
	    try {
	        logger.info(String.format("Run Feature - %s", this.feature));
	        this.runtime.run();
	    } catch (IOException ex) {
	        throw new RuntimeException(ex);
	    }
	    if (!this.runtime.getErrors().isEmpty()) {
	        logger.debug(this.runtime.getErrors().toString());
	        throw new CucumberException(this.runtime.getErrors().get(0));
	    }*/
    return ro;
	}
}
